...
export default function ColdEmailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${backendUrl}/cold-email`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) throw new Error("Server error");
      const data = await res.json();
      setResult(data.cold_email);
    } catch (err) {
        ...
        >
          <Box bg="white" boxShadow="xl" borderRadius="xl" p={8} w="100%">
            ...
            <Text mb={6} color="gray.600">
              Upload your resume and a job description to generate a personalized cold email.
            </Text>
          ...
          >
            <Box p={4} pb={6} mb={4}>
              <Flex align="center" justify="space-between" mb={4} gap={2}>
                ...
                <Clipboard.Root value={result} timeout={1000}>
              ...
              <Box>
                <Box color="gray.700" className="email-content">
                  <div
                      ...
                      __html: result
                        ...
                        .reduce((acc, line) => {
                          if (line.trim().startsWith('•')) {
                            ...
                            if (!acc.includes('<ul>')) {
                              return acc + '<ul><li>' + cleanedLine + '</li>';
                            }
                            return acc + '<li>' + cleanedLine + '</li>';
                          }
                          if (acc.includes('<ul>') && !acc.includes('</ul>')) {
                            return acc + '</ul><p>' + line.trim() + '</p>';
                          }
                          return acc + '<p>' + line.trim() + '</p>';
                        }, '')
                        + (result.split('\n').some(line => line.trim().startsWith('•')) ? '</ul>' : '')
                    }}
                  />
                </Box>
              </Box>
